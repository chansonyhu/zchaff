...
void CSolver::set_variable_number(int n) {
  ...
  //CDatabase::set_variable_number(nt n) {
...
void CSolver::decay_variable_score(void) {
  ...
  //update the scores(qianshan, 7/11/2016)
...
int CSolver::conflict_analysis_firstUIP(void) {
  ...
  if (_conflicts.size() > 1) {
    for (vector<ClauseIdx>::iterator ci = _conflicts.begin();
      ...
      // dlevel() return current decision level(qianshan, 7/11/2016)
      ...
      //update min_conf_length and save the responding clause(qianshan, 7/11/2016)
      ...
      //set the conflict lits' variable's new_cl_phase UNKNOWN(qianshan, 7/11/2016)
